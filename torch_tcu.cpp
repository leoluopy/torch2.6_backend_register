...
at::Tensor custom_empty_memory_format(at::IntArrayRef size, c10::optional<at::ScalarType> dtype, c10::optional<at::Layout> layout, c10::optional<at::Device> device, c10::optional<bool> pin_memory, c10::optional<at::MemoryFormat> memory_format) {
    ...
    std::cout << "Custom aten::empty.memory_format() called! size: " << size << std::endl;
...
// {"schema": "aten::empty_strided(SymInt[] size, SymInt[] stride, *, ScalarType? dtype=None, Layout? layout=None, Device? device=None, bool? pin_memory=None) -> Tensor", "dispatch": "True", "default": "False"}
//at::Tensor empty_strided(c10::SymIntArrayRef size, c10::SymIntArrayRef stride, c10::optional<at::ScalarType> dtype,
//                     c10::optional<at::Layout> layout, c10::optional<at::Device> device, c10::optional<bool> pin_memory){
//
//}
at::Tensor empty_strided(c10::ArrayRef<int64_t> size, c10::ArrayRef<int64_t> stride, c10::optional<at::ScalarType> dtype = c10::nullopt,
                         c10::optional<at::Layout> layout = c10::nullopt,
                         c10::optional<at::Device> device = c10::nullopt,
                         c10::optional<bool> pin_memory = c10::nullopt) {
    std::cout << "Custom aten::empty_strided() called! size:"<< size << std::endl;

    const at::OptionalDeviceGuard device_guard(device);
    constexpr c10::DispatchKeySet private_use_ks(c10::DispatchKey::PrivateUse1);
    at::MemoryFormat memory_format;
    at::Tensor tensor= at::detail::empty_generic(size, &global_custom_alloc, private_use_ks, c10::dtype_or_default(dtype), memory_format);

//    torch::Tensor tensor = torch::empty_strided(size, stride, options);
    return tensor;
}
...
//    at::Tensor cpu_bias = bias.value().to(at::device(c10::kCPU));
    ...
    print_tensor_size(cpu_input);
    print_tensor_size(cpu_weight);
                                                                 ...
                                                                 bool training, double momentum, double eps)
{
    ...
    if (running_mean.has_value()) {
        ...
        print_tensor_size(cpu_running_mean);
    }
    ...
    if (running_var.has_value()) {
        ...
        print_tensor_size(cpu_running_var);
    }
    ...
    print_tensor_size((std::get<0>(output)));
...
{
    printf("Calling TCU OP:  add_out \n");
//    at::Tensor cpu_self = self.to(at::device(c10::kCPU));
    at::Tensor sel_nCon = self ;
//
//
//    at::Tensor cpu_out = out.to(at::device(c10::kCPU));
//
//    torch::add_out(cpu_out,cpu_self,other,alpha);
    return sel_nCon ;
}
at::Tensor & mean_out(const at::Tensor & self, at::OptionalIntArrayRef dim, bool keepdim,
    ...
    at::Tensor cpu_self = self.to(at::device(c10::kCPU));
    at::Tensor cpu_out = out.to(at::device(c10::kCPU));
    std::cout << cpu_self.device();
    print_tensor_size(cpu_self);
    std::cout << cpu_out.device();
    print_tensor_size(cpu_out);

    printf("call torch cpu fun mean show ret shape next\n");
    at::Tensor ret = torch::mean_out(cpu_out,cpu_self,dim,keepdim,dtype);
    std::cout << ret.device();
    print_tensor_size(ret);
...
at::Tensor view(const at::Tensor & self, at::IntArrayRef size){
    ...
    std::cout << "tcu out viewd: "<< tcu_out.device();
    print_tensor_size(tcu_out);
    return tcu_out;
}
...
{
    printf("Calling TCU OP:  as_strided \n");
    at::Tensor cpu_self = self.to(at::device(c10::kCPU));
    std::cout << cpu_self.device();
    print_tensor_size(cpu_self);
    ...
    std::cout << "tcu out as_strided: "<< tcu_out.device();
    print_tensor_size(tcu_out);
    return tcu_out;
}
// {"schema": "aten::addmm.out(Tensor self, Tensor mat1, Tensor mat2, *, Scalar beta=1, Scalar alpha=1, Tensor(a!) out)
...
{
    printf("Calling TCU OP:  addmm_out \n");
    at::Tensor cpu_self = self.to(at::device(c10::kCPU));
    std::cout << cpu_self.device();
    print_tensor_size(cpu_self);
...
torch::Tensor rms_norm_impl(
    ...
    std::cout << "cus backend rms norm" << std::endl;
    return input;
}

// 注册到_C命名空间 (通常用于私有实现)
// if not declared : AttributeError: '_OpNamespace' '_C' object has no attribute 'rms_norm'
TORCH_LIBRARY(_C, m) {
    // 声明算子签名
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("tcu", &get_custom_device, "get custom device object");
}
